        ...
        <div className="flex items-start justify-between gap-2">
          <div>
            <div className="flex items-center gap-2">
              <p className="text-xs font-bold text-slate-800 leading-tight">{stop.name}</p>
              {stopFare && (
            ...
            <p className="text-[10px] text-slate-400 mt-0.5">
              Stop {stop.sequence} · {stop.latitude.toFixed(4)}°N, {stop.longitude.toFixed(4)}°E
            </p>
          </div>
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          {isLoading || !route ? (
            <>
              ...
              <DialogDescription className="sr-only">
                Loading route details and statistics...
              </DialogDescription>
            </>
          ) : (
            <>
              <div className="flex items-center gap-3 mb-1 pr-8">
                ...
                <div>
                  <DialogTitle className="text-xl leading-tight">{route.name}</DialogTitle>
        ...
        {isLoading && (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-sm text-slate-400">Fetching route details…</p>
          </div>
        )}
        ...
        {isError && !isLoading && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm font-medium text-slate-500">Route not found.</p>
            <p className="text-xs text-slate-400 mt-1">
              This route may have been removed or the ID is invalid.
            </p>
          </div>
        )}
        ...
        {isEditing && route && (
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-slate-400 uppercase">Route Name</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-400 uppercase">Estimated Distance (km)</label>
                <Input type="number" step="0.1" value={editForm.estimatedDistance} onChange={e => setEditForm(p => ({...p, estimatedDistance: e.target.value}))} />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-400 uppercase">Estimated Duration (mins)</label>
            ...
            <div className="flex items-center pt-4 border-t border-slate-100">
              ...
              <Button variant="ghost" onClick={() => setIsEditing(false)} disabled={isUpdating || isDeleting}>Cancel</Button>
            ...
            <section>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                ...
                <MetricCard
                  icon={MapPin}
                  label="Total Stops"
                  value={`${route.totalStops} ${route.totalStops === 1 ? "stop" : "stops"}`}
                />
                <MetricCard
                  icon={Banknote}
                  label="Base Fare"
            ...
            <section>
              ...
              {isEditingStops ? (
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-4">
                  <div className="p-3 bg-amber-50 text-amber-700 border border-amber-200 rounded-lg text-xs">
                    <strong>Warning:</strong> Updating stops will permanently delete all existing fares and segments associated with this route.
                  </div>
                        ...
                        <div className="flex-1 space-y-3">
                          <div className="grid grid-cols-12 gap-3">
                            ...
                            <div className="col-span-11 space-y-1.5">
                              <label className="text-[10px] font-bold text-slate-400 uppercase block">Stop Name</label>
                              <Input 
                  ...
                  <div className="flex justify-end gap-2 pt-2">
                    <Button variant="ghost" onClick={() => setIsEditingStops(false)} disabled={isUpdatingStops} size="sm">Cancel</Button>
                    <Button variant="outline" onClick={handleUpdateStops} disabled={isUpdatingStops} className="gap-2" size="sm">
                      ...
                      {isUpdatingStops ? "Saving..." : "Save Stops"}
                    </Button>
                  </div>
                </div>
              ) : isEditingFares ? (
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-4">
                  <div className="p-3 bg-primary/5 text-primary border border-primary/10 rounded-lg text-xs">
                    <strong>Customize Stop-to-Stop Fares:</strong> Update the fares between stops for this route.
                  </div>
                  
                  <div className="space-y-2.5 max-h-[40vh] overflow-y-auto pr-1">
                    {editFares.map((fare, index) => {
                      const fromStopName = route.stops?.find(s => s.sequence === fare.fromStopSequence)?.name || `Stop ${fare.fromStopSequence}`;
                      const toStopName = route.stops?.find(s => s.sequence === fare.toStopSequence)?.name || `Stop ${fare.toStopSequence}`;
                  ...
                  <div className="flex justify-end gap-2 pt-2">
                    <Button variant="ghost" onClick={() => setIsEditingFares(false)} disabled={isUpdatingFares} size="sm">Cancel</Button>
                    <Button variant="outline" onClick={handleUpdateFares} disabled={isUpdatingFares} className="gap-2" size="sm">
                      ...
                      {isUpdatingFares ? "Saving..." : "Save Fares"}
                    </Button>
                  </div>
                </div>
              ) : (
